...
ecoli_type(){
    local run_name repo docker_cmd
    ...
    tail -n+2 SRST2Ec_${run_name}/SRST2_EcOH__fullgenes__EcOH__results.txt | \
    ...
    sed 's/[?*]//g' SRST2Ec_${run_name}/SRST2_ARG__genes__ARGannot__results.txt | \
    ...
    cut -d$'\t' -f 1-9 SRST2Ec_${run_name}/SRST2_Ec1__mlst__Escherichia_coli#1__results.txt | \
        sed 's/[?*]//g; /^$/d' | perl -pe 's/_S[0-9]{1,}_//g; s/_//' | sort | sed 's/ST/ST1/' \
        > SRST2Ec_${run_name}/srst2Ec_${run_name}_achtman#1.tsv

    cut -d$'\t' -f 1-9  SRST2Ec_${run_name}/SRST2_Ec2__mlst__Escherichia_coli#2__results.txt | \
        sed 's/[?*]//g; /^$/d' | perl -pe 's/_S[0-9]{1,}_//g; s/_//' | sort | sed 's/ST/ST2/' \
    ...
    cut -d$'\t' -f 1-9 SRST2Ec_${run_name}/SRST2_LEE__mlst__LEE_mlst__results.txt | \
        sed 's/[?*]//g; /^$/d' | perl -pe 's/_S[0-9]{1,}_//g; s/_//' | tail -n+2 | sort \
        > RESULTS/srst2Ec_${run_name}_LEE.tsv
