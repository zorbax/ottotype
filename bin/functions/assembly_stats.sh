...
assembly_stats_cov() {
    ...
    do
        ...
        awk '!/^>/ { printf "%s", $0; n = "\n" } /^>/' \
                '{ print n $0; n = "" } END { printf "%s", n }' ${i} | \
                sed '/^>/ d'| awk '{ print length($0) }' | sort -gr > ${name}_contig_lengths.stat

        local contigs_number assembly_size
        contigs_number=$(wc -l ${name}_contig_lengths.stat | cut -f1)
        ...
        { echo -e "Number of contigs:\t$contigs_number\n$cov";
          echo -e "Assembly size:\t$assembly_size";
          echo -e "Largest contig:\t$large_contig";
          echo -e "N50:\t$N50\nN90:\t$N90"; } >> ${name}.stats
