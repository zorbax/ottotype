...
if [ $use_docker == true ]; then
  ...
  mapfile -t cleaned < <(find . -maxdepth 1 -name "*cleaned.fastq.gz")
  mapfile -t interleave < <(find . -maxdepth 1 -name "*combo.fastq.gz")

  if [ ${#cleaned[@]} -gt 0 ]; then
    for i in $(cut -d '_' -f1 ./*cleaned.fastq.gz | sort | uniq )
    do
      echo "Add reads $i"
    ...
    for i in $(cut -d '_' -f1 ./*combo.fastq.gz | sort | uniq )
    do
      echo "TrimClean $i"
    ...
    for i in $(cut -d '_' -f1 ./*fastq.gz | sort | uniq )
    do
      echo "Trimming $i"
