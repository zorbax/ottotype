...
if [ $use_docker == true ]; then
  ...
  if [ ! -d "$dir" ]; then
    docker run --rm -it -v $(pwd):/data -u $(id -u):$(id -g) -w /data lyveset set_manage.pl --create set_$(basename `pwd`) 2>/dev/null
  fi
  ...
  if [ ${#cleaned[@]} -gt 0 ]; then
    ...
    do
      echo "Add reads $i"
      docker run --rm -it -v $(pwd):/data -u $(id -u):$(id -g) -w /data lyveset set_manage.pl set_$(basename `pwd`) --add-reads ${i}_cleaned.fastq.gz &> /dev/null
      echo "Done"
    done
  ...
  docker run --rm -it -v $(pwd):/data -u $(id -u):$(id -g) -w /data lyveset set_manage.pl set_$(basename `pwd`) --change-reference "$reference"
  echo "Set"
  docker run --rm -it -v $(pwd):/data -u $(id -u):$(id -g) -w /data lyveset launch_set.pl set_$(basename `pwd`) -ref "$reference" \
  ...
  echo "Set"
  launch_set.pl set_$(basename `pwd`) -ref "$reference" --min_coverage 20 --min_alt_frac 0.95 \
